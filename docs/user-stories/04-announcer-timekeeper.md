...
> from **one shared clock**. Each group runs an **automatic phased sequence**:
> announce round/group → announce pilots → **preparation** → **working time** →
> **landing window** → a short **inter-group gap** before the next group.
...
- **Area 6.4** — Round Progression: advance to the next round, **gated** by the
  previous round being fully scored; this also bounds the Scorer correction window
  ([Area 5](../requirements/high-level-requirements.md#area-5--scoring)).
- **Area 6.5** — Group Run Control: start groups and let them progress
  automatically; the **Contest Director** holds the authority actions
  (prep pause/fast-forward/add-time, prep-gate override, abort).
...
- [ ] Given the working-time countdown reaches zero, when it ends, then it clearly
  marks the **end of working time** — the point beyond which flight time is no
  longer accrued — and for classes whose rules require it, **all flight-time timing
...
- [ ] Given the working time has ended, when the **landing window** runs, then all
  competitors must land within it; its overrun **consequence is recorded by the
  Scorer** as part of the flight
  ([5.2.3](03-scorer.md#523--record-task-integral-deductions-the-flight-incurs)),
  not auto-computed by the timer.
...
that** no flight is left unscored and Scorers keep a clear window to fix their own
entries until the moment the next round begins.
...
- [ ] Given a round in which **every group has all its scores captured** (and no
  no-score is still resolvable), when I advance, then the next round becomes the
  current round and its first group can be run.
- [ ] Given the moment I start the next round, when it begins, then the previous
  round is **closed to Scorer self-correction** — from here a change to a previous
  round's value is mid-contest score administration for the
  ...
  not the Scorer's ([03-scorer 5.1.2](03-scorer.md#512--eyes-on-the-flight-capture-with-obvious-mistypes)).
...
Starting each **group** within a round is not gated the same way — groups start
automatically after the inter-group gap ([6.5](#area-65--group-run-control)),
subject to the prep confirmation gate. Assigned to the Announcer/Timekeeper per the
...
### 6.5.1 — Start groups and let them progress automatically

**As an** Announcer / Timekeeper, **I want** to start a group and have the sequence
run — and the next group follow automatically — **so that** I can run one group
after another with my hands busy and my attention on the field.
...
- [ ] Given a group has finished its **landing window**, when the configured
  **inter-group gap** elapses ([3.8](../requirements/high-level-requirements.md#area-3--competition-setup--configuration)),
  then the **next group starts automatically** — I do not have to start each group
  by hand.
- [ ] Given the **prep confirmation gate** is holding a group's preparation at one
  minute remaining because not every Scorer has confirmed their pilot
...
1. **Area 6 confirmed — RESOLVED.** Sub-areas 6.1 Timer, 6.2 Audio, 6.3 Field
   ...
   phased sequence (prep → working → landing → inter-group gap), and the
   Contest-Director run control are confirmed and recorded in
   [`high-level-requirements.md` Area 6](../requirements/high-level-requirements.md#area-6--display-timer--audio-field-aids).
...
5. **Round-progression ownership — RESOLVED, kept for record.** Round progression
   ...
   Only **round** starts are gated; **group** starts within a round run
   automatically after the inter-group gap, subject to the prep confirmation gate.
   Recorded as [Area 6.4](../requirements/high-level-requirements.md#area-6--display-timer--audio-field-aids)
   and [6.5](#area-65--group-run-control).
